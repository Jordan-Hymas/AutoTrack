...
export function getMaintenanceStats(vehicle, nowMs = Date.now(), options = {}) {
  const now = Number.isFinite(nowMs) ? nowMs : Date.now();
  const honda2017TestMode = options?.honda2017TestMode === true;
  ...
  if (honda2017TestMode && vehicle?.id === TEST_PROGRESS_VEHICLE_ID) {
...
export function reducer(state, action) {
  ...
  if (action.type === "update-odometer") {
    ...
    const parsedOdometer = Number(action.odometer);
    if (!Number.isFinite(parsedOdometer)) {
      throw new Error("Enter a valid odometer reading.");
    }
    const nextOdometer = Math.max(0, Math.round(parsedOdometer));
  ...
  if (action.type === "update-intervals") {
    ...
    const parsedOilInterval = Number(action.oilInterval);
    const parsedTireInterval = Number(action.tireInterval);
    const parsedOilIntervalMonths = Number(
      action.oilIntervalMonths ?? target.oilIntervalMonths
    );
    const parsedTireIntervalMonths = Number(
      action.tireIntervalMonths ?? target.tireIntervalMonths
    );
    if (
      ...
      throw new Error("Enter valid interval values.");
    }
    ...
    if (
      oilInterval === target.oilInterval &&
      tireInterval === target.tireInterval &&
      oilIntervalMonths === target.oilIntervalMonths &&
      tireIntervalMonths === target.tireIntervalMonths
    ) {
      return state;
    }
    ...
    const vehicles = state.vehicles.map((item) =>
      item.id === action.vehicleId
        ? { ...item, oilInterval, tireInterval, oilIntervalMonths, tireIntervalMonths, updatedAt: stamp() }
        : item
    );
    ...
    return {
      ...
      history: [
        logEntry({
          ...
          details: `Intervals set to oil ${oilInterval} mi/${oilIntervalMonths} mo, tires ${tireInterval} mi/${tireIntervalMonths} mo`
        }),
        ...state.history
      ]
    };
  ...
  if (action.type === "reset-vehicle-data") {
    const target = state.vehicles.find((item) => item.id === action.vehicleId);
    if (!target) throw new Error("Vehicle not found.");
    const resetAt = stamp();
    ...
    const vehicles = state.vehicles.map((item) =>
      item.id === action.vehicleId
        ? {
            ...
            lastTireRotationOdometer: 0,
            lastOilChangeDateISO: resetAt,
            lastTireRotationDateISO: resetAt,
            serviceOverrides: { oil_change: null, tire_rotation: null },
            updatedAt: resetAt
          }
        : item
    );
    ...
    return {
      ...state,
      vehicles,
      history: state.history.filter((entry) => entry.vehicleId !== action.vehicleId)
    };
  }
