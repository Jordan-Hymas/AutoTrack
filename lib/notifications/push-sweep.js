...
async function sendPayloadToSubscriptions(payload, subscriptions) {
  ...
  return { sentCount, failedCount, removedCount, errors };
}
...
export async function runPushNotificationSweep(options = {}) {
  ...
  const honda2017TestMode = snapshot?.settings?.pwaSettings?.honda2017TestMode === true;
  ...
  for (const vehicle of vehicles) {
    const stats = getMaintenanceStats(vehicle, nowMs, { honda2017TestMode });
    ...
    for (const item of checksForVehicle) {
      ...
      if (sendResult.errors.length > 0 && errorSamples.length < 6) {
        errorSamples.push(...sendResult.errors.slice(0, 6 - errorSamples.length));
      }
