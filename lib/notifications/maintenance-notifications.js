...
export function getNotificationStage(remainingMs, windowMs) {
  const normalizedWindowMs = Number.isFinite(windowMs) && windowMs > 0 ? windowMs : Number.POSITIVE_INFINITY;
  const notifyLeadMs = Math.min(7 * DAY_MS, Math.max(60 * 1000, normalizedWindowMs / 3));
  if (remainingMs < -DAY_MS) return "overdue";
  if (remainingMs <= 0) return "service_due";
  if (remainingMs <= notifyLeadMs) return "time_to_service";
  return null;
}

export function getNotificationStageKey(stage, dueDateISO) {
  const dueDateKey = Number.isFinite(Date.parse(dueDateISO))
    ? new Date(dueDateISO).toISOString().slice(0, 10)
    : "unknown";
